...
const slashCommands = [
    ...
    // -- Personal vouch count check ----------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("vouchcount")
            .setDescription("Check your own vouch count")
            .addUserOption(o => o.setName("user").setDescription("Check someone else instead").setRequired(false)),
        async execute(interaction){
            const target = interaction.options.getUser("user") || interaction.user;
            const users = usersDB.read();
            const count = users[target.id]?.vouches || 0;

            await interaction.reply({ content: `${target.id === interaction.user.id ? "You have" : `${target.username} has`} **${count}** vouch${count === 1 ? "" : "es"}.`, ephemeral: true });
        }
    },
...
async function handleLeaveVouchMessage(message){
    ...
    // If deleting a message fails, this is almost always a missing "Manage
    // Messages" permission for the bot in this specific channel. That's been
    // silently logged to Railway only, which nobody was checking - post a
    // short-lived warning directly in the channel instead so it's obvious.
    async function tryDelete(msg, label){
        try{
            await msg.delete();
        }catch(err){
            console.log(`[leave-vouch] ❌ could not delete ${label}: ${err.message}`);
            const warning = await msg.channel.send({
                content: `⚠️ Couldn't delete a ${label} here (\`${err.message}\`) - the bot's role likely needs **Manage Messages** in this channel. Ask staff to check.`
            }).catch(() => null);
        ...
        if(pending && Date.now() - pending.timestamp < CONFIG.VOUCH_PHOTO_WINDOW_MS){
            ...
            if(posted){
                if(pending.textMessageId){
                    ...
                    if(original) await tryDelete(original, "vouch text message");
                }
                await tryDelete(message, "vouch photo");
            }
    ...
    const timer = setTimeout(async () => {
        ...
        if(original) await tryDelete(original, "expired vouch message");
