...
// Looks up a custom emoji by ID directly from the bot's cache and builds the
// correct <a:name:id> (animated) or <:name:id> (static) tag automatically -
// no more guessing whether an emoji is animated. If the bot can't see the
// emoji at all (wrong ID, or it's from a server the bot isn't in), logs a
// clear warning and falls back to plain text instead of a broken tag.
function emojiTag(client, id, fallbackLabel){
    const emoji = client.emojis.cache.get(id);
    if(emoji) return emoji.toString();
    console.log(`[emoji] ⚠️ could not find emoji ${id} in the bot's cache - is the bot actually in the server that owns this emoji? Falling back to plain text.`);
...
const slashCommands = [
    ...
    // -- Champion's Quest (Nuke) pricing panel -------------------------------
    {
        ...
        async execute(interaction){
            ...
            const nukeEmoji = emojiTag(interaction.client, "1528171131700248676", "☢️");
            const nukeGuildEmoji = interaction.client.emojis.cache.get("1528171131700248676");
    ...
    // -- Ranked Play pricing panel --------------------------------------------
    {
        ...
        async execute(interaction){
            ...
            const ranks = {
                bronze: emojiTag(interaction.client, "1491549809461694734", "Bronze"),
                silver: emojiTag(interaction.client, "1523088147326566652", "Silver"),
                gold: emojiTag(interaction.client, "1523088229106974790", "Gold"),
                platinum: emojiTag(interaction.client, "1472692456628813900", "Platinum"),
                diamond: emojiTag(interaction.client, "1472692273421750403", "Diamond"),
                crimson: emojiTag(interaction.client, "1472692153837813957", "Crimson"),
                iridescent: emojiTag(interaction.client, "1472692082882641960", "Iridescent"),
                top250: emojiTag(interaction.client, "1503807783303249960", "⭐")
            };
            const arrow = emojiTag(interaction.client, "1494644832554061825", "→");
