...
const { getShoukaku } = require("./music-manager");
...
const slashCommands = [
    ...
    // -- Rank Calculator (replicates the website calculator) ------------------
    {
        ...
        async execute(interaction){
            ...
            const state = { currentIndex: null, desiredIndex: null };

            function buildCurrentSelect(){
                const menu = new StringSelectMenuBuilder()
                    .setCustomId("rankcalc_current")
                    .setPlaceholder("Current Rank");

                const options = RANK_TIERS.map((tier, i) => {
                    const opt = { label: tier.price >= 500 ? `${tier.name} — $500+` : tier.name, value: String(i) };
            ...
            function buildDesiredSelect(){
                const menu = new StringSelectMenuBuilder().setCustomId("rankcalc_desired");

                const valid = state.currentIndex === null
                    ? RANK_TIERS.map((_, i) => i)
                    : RANK_TIERS.map((_, i) => i).filter(i => i > state.currentIndex);

                if(!valid.length){
                    menu.setPlaceholder("No higher ranks available").setDisabled(true)
                        .addOptions([{ label: "No higher ranks available", value: "none" }]);
                    return menu;
                }

                menu.setPlaceholder("Desired Rank").setDisabled(state.currentIndex === null);

                const options = valid.map(i => {
            ...
            function buildResultEmbed(){
                const current = state.currentIndex !== null ? RANK_TIERS[state.currentIndex] : null;
                const desired = state.desiredIndex !== null ? RANK_TIERS[state.desiredIndex] : null;

                let description;

                if(!current || !desired){
                    description = "Pick a **current rank** and a **desired rank** below to see your price.";
                }else if(state.desiredIndex <= state.currentIndex){
                    description = "⚠️ **Select a higher desired rank**";
                }else{
                    const price = calcRankPrice(state.currentIndex, state.desiredIndex);
                    const rankCount = state.desiredIndex - state.currentIndex;
                ...
                return brandEmbed({ title: "Rank Calculator", description, thumbnail: false });
            }

            function buildComponents(){
                const row1 = new ActionRowBuilder().addComponents(buildCurrentSelect());
                const row2 = new ActionRowBuilder().addComponents(buildDesiredSelect());
                const row3 = new ActionRowBuilder().addComponents(
                    new ButtonBuilder()
                        .setLabel("Order Rank Boost")
                        .setStyle(ButtonStyle.Link)
                        .setURL("https://sinners-vouches-production.up.railway.app")
                );
            ...
            const collector = message.createMessageComponentCollector({
                filter: (i) => i.user.id === interaction.user.id && ["rankcalc_current", "rankcalc_desired"].includes(i.customId),
                time: 5 * 60000
            });

            collector.on("collect", async (i) => {
                if(i.customId === "rankcalc_current"){
                    state.currentIndex = parseInt(i.values[0]);
                    // Changing current can invalidate the previously picked desired rank
                    if(state.desiredIndex !== null && state.desiredIndex <= state.currentIndex){
                        state.desiredIndex = null;
                    }
                }else if(i.customId === "rankcalc_desired" && i.values[0] !== "none"){
                    state.desiredIndex = parseInt(i.values[0]);
                }

                await i.update({ embeds: [buildResultEmbed()], components: buildComponents() });
            });
    ...
    // -- FAQ ------------------------------------------------------------------
    {
        ...
        async execute(interaction){
            ...
            const menu = new StringSelectMenuBuilder()
                ...
                .addOptions(
                    { label: "How long does a boost take?", value: "faq_time", emoji: "⏱️" },
                    { label: "Is my account safe?", value: "faq_safety", emoji: "🔒" },
                    { label: "What payment methods do you accept?", value: "faq_payment", emoji: "💳" },
                    { label: "Do you offer refunds?", value: "faq_refunds", emoji: "↩️" },
                    { label: "How does number rental work?", value: "faq_numbers", emoji: "📱" }
                );
    ...
    // -- Price calculator (mirrors the website's Calculator page) -------------
    {
        data: new SlashCommandBuilder()
            .setName("quote")
            .setDescription("Get an instant price quote")
            .addStringOption(o => o.setName("service").setDescription("Which service").setRequired(true)
                .addChoices(
                    { name: "Nuke (1x)", value: "nuke_1" },
                    { name: "Nuke (5x)", value: "nuke_5" },
                    { name: "Nuke (10x)", value: "nuke_10" },
                    { name: "Rank Boost: Bronze → Silver", value: "rank_1" },
                    { name: "Rank Boost: Silver → Gold", value: "rank_2" },
                    { name: "Rank Boost: Gold → Platinum", value: "rank_3" },
                    { name: "Rank Boost: Platinum → Diamond", value: "rank_4" },
                    { name: "Rank Boost: Diamond → Crimson", value: "rank_5" },
                    { name: "Rank Boost: Crimson → Iridescent", value: "rank_6" },
                    { name: "Rank Boost: Bronze → Iridescent (full)", value: "rank_full" }
                )),

        async execute(interaction){
            const prices = {
                nuke_1: 79.99, nuke_5: 199.99, nuke_10: 399.99,
                rank_1: 10, rank_2: 30, rank_3: 50, rank_4: 65, rank_5: 90, rank_6: 145,
                rank_full: 250
            };
            ...
            const embed = brandEmbed({
                title: "🧮 Price Quote",
                fields: [
                    { name: "Service", value: interaction.options.getString("service").replace(/_/g, " ") },
                    { name: "Price", value: `$${price.toFixed(2)}` },
                    { name: "Next step", value: "Open a ticket to lock in this price and start your order." }
                ]
            });
    ...
    // -- Self-diagnostic status command (the "surprise") --------------------
    {
        ...
        async execute(interaction){
            ...
            checks.push([`LAVALINK_HOST`, !!process.env.LAVALINK_HOST]);
            checks.push([`LAVALINK_PORT`, !!process.env.LAVALINK_PORT]);
            checks.push([`LAVALINK_PASSWORD`, !!process.env.LAVALINK_PASSWORD]);
            ...
            let lavalinkStatus = "❌ music system not initialized";
            const shoukaku = getShoukaku();
            if(shoukaku){
                const nodes = [...shoukaku.nodes.values()];
                if(!nodes.length){
                    lavalinkStatus = "❌ no nodes configured";
                }else{
                    lavalinkStatus = nodes.map(n => `${n.state === 1 ? "✅" : "❌"} ${n.name} (state: ${["CONNECTING", "CONNECTED", "DISCONNECTING", "DISCONNECTED"][n.state] || n.state})`).join("\n");
                }
            }
            ...
            const embed = brandEmbed({
                ...
                fields: [
                    ...
                    { name: "🎵 Lavalink Node(s)", value: lavalinkStatus },
...
async function createTicketFor(interaction, choice){
    ...
    const log = ticketLogDB.read();
    log.open.push({
        channelId: channel.id, user: interaction.user.id, service: choice,
        created: new Date().toISOString(), status: "pending", claimedBy: null, closedBy: null
    });
    ticketLogDB.write(log);
...
const selectHandlers = {
    ...
    async faq_select(interaction){
        const answers = {
            faq_time: "Most boosts complete within a few hours depending on rank distance and queue. Rank Boosts are typically same-day; Nuke orders are scheduled with you directly since they're manually piloted via Battlenet.",
            faq_safety: "We never ask for your password over chat outside of an active, staff-claimed ticket, and boosts are done carefully to avoid unnecessary risk. If anything feels off, stop and open a ticket immediately.",
            faq_payment: "Check the payment-methods channel for current accepted methods - these can change, so that channel is always the source of truth.",
            faq_refunds: "Refunds are handled case-by-case through a ticket - if a service can't be delivered as described, staff will work it out with you there.",
            faq_numbers: "Use `/getnumber` or `/gen` to rent a temporary number for SMS verification. You'll get a real phone number, a code shows up when the target site texts it, and the order auto-refunds if no code arrives in time."
        };

        const embed = brandEmbed({
            title: "❓ FAQ Answer",
            description: answers[interaction.values[0]] || "No answer found for that topic - please open a ticket instead."
        });
