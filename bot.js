...
// ---------------------------------------------------------------------------
// MUSIC SYSTEM - everything lives under ./music, this is the only spot in
// bot.js that needs to know it exists.
// ---------------------------------------------------------------------------
const {
    initLavalink,
    musicSlashCommands,
...
const client = new Client({
    intents: [
        ...
        GatewayIntentBits.GuildVoiceStates // required for the music system to join/track voice channels
    ]
});

// Build a quick lookup map: commandName -> execute()
// (spread music commands in alongside your existing ones - same {data, execute} shape)
const commandMap = new Map(
    [...slashCommands, ...musicSlashCommands].map(c => [c.data.name, c.execute])
);

// Same idea for buttons - music button customIds (music_*) are unique so
// they can't collide with your existing ones.
...
client.once("clientReady", () => {
    ...
    // Connect to Lavalink. client.shoukaku is then reachable from anywhere
    // that has the interaction's client (i.e. every music command/button).
...
client.on("interactionCreate", async (interaction) => {
    ...
    try{
        ...
        // Autocomplete (e.g. typing into /play) is a distinct interaction type
        // from a normal slash command submission - handle it first.
        if(interaction.isAutocomplete()){
            if(interaction.commandName === "play"){
                await musicAutocomplete(interaction);
            }
            return;
        }
        ...
        if(interaction.isButton()){
            const handler = allButtonHandlers[interaction.customId];
            if(handler){
        ...
        // Autocomplete interactions can only be responded to with .respond(),
        // not .reply()/.followUp() - bail out here instead of throwing again.
        if(interaction.isAutocomplete()){
...
client.on("voiceStateUpdate", async (oldState, newState) => {
    try{
        ...
        console.error("[music] voiceStateUpdate cleanup error:", error);
    }
});
