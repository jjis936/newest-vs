...
# ^ strongly recommended - security alerts (invite spam blocked, mass
#   mention spam, anti-nuke triggers, new/young accounts joining) post here.
#   Leave blank and these still work, they just won't be logged anywhere.

NEW_ACCOUNT_MIN_AGE_DAYS=3
# ^ accounts younger than this get flagged (not auto-kicked) when they join

MAX_MENTIONS_PER_MESSAGE=6
# ^ messages with more mentions than this get deleted + a 10min timeout
#   (staff with Manage Server are exempt)

ANTI_NUKE_ENABLED=true
# ^ set to false to disable. When enabled: if a non-owner deletes 3+
#   channels/roles within 10 seconds, their roles get stripped and staff
#   are alerted - protects against a compromised admin/mod account
