...
async function resolveQuery(query, requesterId){
    ...
    if(isUrl(query)){
        ...
        // fall back to YouTube, then SoundCloud (SoundCloud is natively
        // supported by every Lavalink node with zero extra config, and
        // YouTube search on vanilla Lavalink has become unreliable lately
        // due to YouTube's anti-bot measures unless a node has the special
        // youtube-source plugin with OAuth/PoToken set up).
...
const musicCommands = [
...

    {
        ...
        async execute(interaction){
            ...
            const more = playlist.tracks.length > 25 ? `\n*(+${playlist.tracks.length - 25} more)*` : "";

            await interaction.reply({
                content: `**📁 ${name}** (${playlist.tracks.length} track(s))\n${lines}${more}`,
                ephemeral: true
            });
        },
