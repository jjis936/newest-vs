...
// embed, and handles clicks on them.
...
// Makes sure a queue exists and user is in the same voice channel
...
const musicButtonHandlers = {
    ...
    async music_shuffle(interaction) {
        ...
        if (queue.tracks.length < 2) {
            return interaction.reply({
                content: "❌ Not enough songs queued to shuffle.",
                ephemeral: true
            });
        }
    ...
    async music_loop(interaction) {
        ...
        await interaction.reply({
            content: `🔁 Loop mode: **${loopLabel(queue.loop)}**`,
            ephemeral: true
        });
    },
    ...
    async music_like(interaction) {
        ...
        if (queue.likes.has(interaction.user.id)) {
            ...
            await interaction.reply({
                content: "💔 Removed from your likes.",
                ephemeral: true
            });
