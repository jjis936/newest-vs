...
const { getIdealNode, joinVoiceChannel, leaveVoiceChannel } = require("./music-manager");
const { getQueue, createQueue, deleteQueue, LOOP_OFF, LOOP_TRACK, LOOP_QUEUE } = require("./music-queue");
const { buildNowPlayingEmbed, buildMusicButtons, formatTime } = require("./music-embeds");
const { getUserPlaylists, getPlaylist } = require("./music-playlists");
...
async function postNowPlaying(queue, channel, brandName, brandColor){
    // Disable the buttons on the PREVIOUS now-playing message, so old
    // messages don't leave clickable buttons acting on a stale track.
    // Fire-and-forget (not awaited) - this was blocking every single song
    // transition on 2 extra API round trips before the NEXT song's message
    // could even post, adding real, avoidable delay to every track change.
    if(queue.nowPlayingChannelId && queue.nowPlayingMessageId){
        channel.client.channels.fetch(queue.nowPlayingChannelId)
            .then(oldChannel => oldChannel?.messages.fetch(queue.nowPlayingMessageId))
            .then(oldMsg => oldMsg?.edit({ components: [] }))
            .catch(() => {}); // old message may already be gone - fine, don't block on it
    }
...
function startProgressUpdater(queue, client, brandName, brandColor){
    ...
    const interval = setInterval(async () => {
        ...
        // WATCHDOG: if the current track's length has been exceeded by a
        // large margin and playback never advanced, the "end" event almost
        // certainly never fired (a Lavalink/Shoukaku event delivery hiccup) -
        // this is what "queue just stops working" looks like. Force the
        // advance manually instead of leaving the bot silently stuck.
        const elapsed = Date.now() - stillQueue.startedAt;
        const trackLength = stillQueue.current.info.length;
        const isLiveStream = stillQueue.current.info.isStream;

        if(!isLiveStream && trackLength > 0 && elapsed > trackLength + 15000){
            console.log(`🎵 🚨 WATCHDOG: guild ${queue.guildId} stuck on "${stillQueue.current.info.title}" - elapsed ${elapsed}ms exceeds track length ${trackLength}ms by >15s. Forcing advance since the "end" event apparently never fired.`);
            stopProgressUpdater(queue.guildId);
            await handleTrackEnd(queue.guildId, client.channels.cache.get(stillQueue.nowPlayingChannelId) || await client.channels.fetch(stillQueue.nowPlayingChannelId).catch(() => null), brandName, brandColor);
            return;
        }
...
const musicCommands = [

    {
        ...
        async execute(interaction, ctx){
            ...
            if(!queue){
                ...
                // Defense-in-depth: guarantee these listeners are NEVER
                // attached twice to the same player, even if this code path
                // somehow runs again for a player that already has them -
                // duplicate listeners would cause erratic double-skips.
                if(queue.player._musicListenersAttached){
                    console.log(`🎵 ⚠️ Listeners already attached for guild ${interaction.guild.id} - skipping duplicate attachment`);
                }else{
                    queue.player._musicListenersAttached = true;

                    queue.player.on("end", (data) => {
                        console.log(`🎵 [end event fired] guild ${interaction.guild.id} reason="${data?.reason}" track="${data?.track?.info?.title || "unknown"}"`);
                        ...
                        // "replaced" fires when OUR OWN code calls playTrack again
                        // (e.g. /skip, /previous) - the replacing code already
                        // handles its own advance, so don't double-advance here.
                        if(data?.reason === "replaced"){
                            console.log(`🎵 [end event] reason=replaced, skipping auto-advance (already handled by whatever triggered the replace)`);
                            return;
                        }
        ...
        async autocomplete(interaction){
            ...
            // IMPORTANT: only ONE search here, not a cascade. Discord fires
            // this on nearly every keystroke while typing - 3 sequential
            // Lavalink calls per keystroke was hammering the node hard
            // enough to cause real audio lag for anyone currently listening.
            // Using SoundCloud specifically (not YouTube) also matters: the
            // suggestion's value becomes a direct URL, and URL playback has
            // no fallback chain - a YouTube suggestion would frequently fail
            // to actually play when selected, since that's the source with
            // known streaming issues. SoundCloud results actually play.
            try{
                const result = await node.rest.resolve(`scsearch:${focused}`);
                const tracks = result?.loadType === "search" ? result.data : [];
    ...
    {
        data: new SlashCommandBuilder().setName("remove").setDescription("Remove a specific song from the queue")
            .addIntegerOption(o => o.setName("position").setDescription("Queue position (from /queue)").setRequired(true)),
        async execute(interaction){
            const access = checkVoiceAccess(interaction);
            if(!access.ok) return interaction.reply({ content: access.reason, ephemeral: true });
            if(!access.queue?.tracks.length) return interaction.reply({ content: "❌ The queue is empty.", ephemeral: true });

            const position = interaction.options.getInteger("position");
            const removed = access.queue.remove(position);

            if(!removed){
                return interaction.reply({ content: `❌ No track at position ${position}. Use \`/queue\` to see valid positions.`, ephemeral: true });
            }

            await interaction.reply({ content: `🗑️ Removed **${removed.info.title}** from the queue.` });
        }
    },

    {
        data: new SlashCommandBuilder().setName("clearqueue").setDescription("Clear the upcoming queue without stopping the current song"),
        async execute(interaction){
            const access = checkVoiceAccess(interaction);
            if(!access.ok) return interaction.reply({ content: access.reason, ephemeral: true });
            if(!access.queue?.tracks.length) return interaction.reply({ content: "❌ The queue is already empty.", ephemeral: true });

            const count = access.queue.tracks.length;
            access.queue.clear();
            await interaction.reply({ content: `🧹 Cleared ${count} track(s) from the queue. Current song keeps playing.` });
        }
    },
